...
class MultiTimeSeriesTransformer(nn.Module):
    def __init__(
        ...
        self.sales_encoder = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(
                d_model=self.total_embedding_dim, nhead=nhead, dim_feedforward=dim_feedforward, dropout=dropout
            ),
    ...
    def forward(
        ...
        moy: torch.Tensor,
        qoy: torch.Tensor,
        sales_padding_mask: torch.Tensor,
        price_padding_mask: torch.Tensor,
        price_validity_mask: torch.Tensor,
        client: torch.Tensor,
        warehouse: torch.Tensor,
        product: torch.Tensor,
        rolling_4w_sales: torch.Tensor,
        rolling_13w_sales: torch.Tensor,
    ) -> torch.Tensor:
        # Replace NaN values in inputs with neutral values
        sales = torch.nan_to_num(sales, nan=0.0)
        price = torch.nan_to_num(price, nan=0.0)
        rolling_4w_sales = torch.nan_to_num(rolling_4w_sales, nan=0.0)
        rolling_13w_sales = torch.nan_to_num(rolling_13w_sales, nan=0.0)
        ...
        sales_emb = torch.cat(
            [
                sales_emb,
                wom_emb[:, : sales_emb.size(1)],
                woy_emb[:, : sales_emb.size(1)],
                moy_emb[:, : sales_emb.size(1)],
                qoy_emb[:, : sales_emb.size(1)],
                cat_emb.unsqueeze(1).repeat(1, sales_emb.size(1), 1),
            ],
            dim=-1,
        )

        price_emb = torch.cat(
            [price_emb, wom_emb, woy_emb, moy_emb, qoy_emb, cat_emb.unsqueeze(1).repeat(1, price_emb.size(1), 1)],
            dim=-1,
        )
        ...
        decoder_emb = torch.cat(
            [
                decoder_emb,
                wom_emb[:, -decoder_emb.size(1) :],
                woy_emb[:, -decoder_emb.size(1) :],
                moy_emb[:, -decoder_emb.size(1) :],
                qoy_emb[:, -decoder_emb.size(1) :],
                cat_emb.unsqueeze(1).repeat(1, decoder_emb.size(1), 1),
            ],
            dim=-1,
        )
        ...
        sales_encoded = self.sales_encoder(
            sales_emb.permute(1, 0, 2), mask=causal_mask_sales, src_key_padding_mask=(~sales_padding_mask)
        )
        price_encoded = self.price_encoder(
            price_emb.permute(1, 0, 2), mask=causal_mask_price, src_key_padding_mask=(~final_price_mask)
        )
        ...
        # Decode future sales
        decoder_output = self.decoder(decoder_emb.permute(1, 0, 2), memory, tgt_mask=None, tgt_key_padding_mask=None)

        # Output predictions
        output = self.output_layer(decoder_output.permute(1, 0, 2))
        future_predictions: torch.Tensor = output[:, -13:, :]  # Shape: (batch_size, max_future_weeks, 1)
    ...
    @staticmethod
    def masked_loss(
        predictions: torch.Tensor, target: torch.Tensor, loss_fn: nn.Module = nn.MSELoss(reduction="none")
    ) -> torch.Tensor:
        """
        ...
        loss_value: torch.Tensor = masked_loss.sum() / (mask.sum() + 1e-8)
        return loss_value

    @staticmethod
    def masked_competition_loss(predictions: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        """
        Custom loss matching the competition metric:
            score = ( sum |pred_i - true_i| + | sum(pred_i - true_i) | ) / sum(true_i)

        Args:
            predictions: Shape (batch_size, seq_len, 1) or (batch_size, seq_len)
            target: Shape (batch_size, seq_len)

        Returns:
            torch.Tensor: Competition loss value
        """
        ...
        competition_loss_value: torch.Tensor = total_error / (sum_of_actuals + 1e-8)
